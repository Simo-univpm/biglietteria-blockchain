...
router.get('/annullatori', verifyToken, (req, res, next) => checkPrivileges(req, res, ["Staff biglietteria"], next), verifyOTP, async (req, res) => {

  var result = await userController.getAnnullatori();  //Ottiene la lista di tutti gli utenti registrati
  new Staff_biglietteria.ListaAnnullatori(req.user.email,req.query.id,result[1]).send(res)
});
...
router.get('/biglietti', verifyToken,(req, res, next) => checkPrivileges(req, res, ["Organizzatore eventi","Staff biglietteria"], next), async (req, res) => {
  ...
  eval('new '+req.user.privileges.replace(" ","_")+'.ListaBiglietti(req.user.email,result[1]).send(res)')
});

router.get('/ricevute', verifyToken,(req, res, next) => checkPrivileges(req, res, ["Staff biglietteria"], next), async (req, res) => {
    
  var result = await receiptController.getReceiptByEvent(req.query.id);
  new Staff_biglietteria.ListaRicevute(req.user.email,result[1]).send(res)
});

router.get('/modifica-password', verifyToken,(req, res, next) => checkPrivileges(req, res, ["Cliente","Organizzatore eventi","Staff biglietteria","Annullatore"], next), async (req, res) => eval('new '+req.user.privileges.replace(" ","_")+'.ModificaPassword(req.user.email).send(res)'));
