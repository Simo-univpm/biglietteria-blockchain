...
function createHTMLPage(res,titolo,descrizione,pagina,moduli)
{
  ...
  res.write('<head><title>'+titolo+'</title><meta name="description" content='+descrizione+'></meta><link rel="stylesheet" href="Librerie/stili.css"></link>');
  for(let i=0;i<moduli.length;i+=1)
    res.write('<script src="Librerie/'+moduli[i]+'.js"></script>')
  res.write('<script src="'+pagina+'.js"></script></head>')
  res.write('<body id="body"><script>create'+pagina+'Page()</script></body>');
  res.end('</html>');
}
