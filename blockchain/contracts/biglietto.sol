...
contract Biglietteria {
    ...
    constructor(string memory _titoloEvento, string memory _luogo, string memory _artisti, uint _postiTotali, Data memory _data_evento, Ora memory _orario_evento){
        ...
        controllaData(_data_evento);
        controllaOra(_orario_evento);
    ...
    function controllaData(Data memory _data_evento) private view returns (bool){
        require(_data_evento.mese<=12);
        require(_data_evento.giorno<=giorni_per_mese[_data_evento.mese-1]);
        require(_data_evento.anno>=2021);
        return true;
    }
    
    function controllaOra(Ora memory _orario_evento) private pure returns (bool){
        require(_orario_evento.ora<24);
        require(_orario_evento.minuti<60);
        require(_orario_evento.secondi<60);
        return true;
    }
