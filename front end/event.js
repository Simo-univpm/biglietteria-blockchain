...
class Event extends Widget{
    ...
    constructor(evento){
        ...
        immagine.setAttribute("src",evento.Icona_evento)  //Viene impostato il percorso da cui leggere l'immagine
    ...
    addData(attributi){
        ...
        for (let i=0; i<attributi.length; i++){
            if (this.evento[attributi[i]]!="Nessuno"){
                const details = new Widget("p",attributi[i].charAt(0).toUpperCase()+attributi[i].slice(1).replace("_"," ")+": "+this.evento[attributi[i]])  //Crea un tag di tipo p
